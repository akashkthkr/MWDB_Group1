...
class DecisionTreeHandler3:
    def __init__(self, task_id=None, train_data=None, test_data=None):
        ...
        self.image_sample_ids_list = self.get_labels()
    ...
    def execute(self):
        ...
        for original_sample_id in self.image_sample_ids_list:
            ...
            for image_id in self.train_data:
                ...
                if subject_id == original_sample_id:
        ...
        jsonFile = open(OUTPUTS_PATH + "DT_Classifier_Values_Task3.json", "w")
        jsonFile.write(jsonString)
        jsonFile.close()

        jsonString = json.dumps(negative_image_assosciation, cls=NumpyEncoder)
        jsonFile = open(OUTPUTS_PATH + "DT_Classifier_negative_Values_Task3.json", "w")
        jsonFile.write(jsonString)
        jsonFile.close()

        jsonString = json.dumps(classifier_results, cls=NumpyEncoder)
        jsonFile = open(OUTPUTS_PATH + "DT_Classifier_Results_Task3.json", "w")
        jsonFile.write(jsonString)
        jsonFile.close()
        ...
        svm_utilities = DTUtilities(self.false_positives, self.true_positives, self.false_negatives,
                                     ...
                                     self.test_set_features, classifier_results)
