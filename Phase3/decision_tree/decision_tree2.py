...
class DecisionTreeClassifier:
    ...
    def splitting_best1(self, X, y):
        m = y.size
        if m <= 1:
            return None, None
        parent_number = [np.sum(y == c) for c in self.unique_classes]
        best_gini = 1.0 - sum((n / m) ** 2 for n in parent_number)
        index_best, best_limit = None, None
        for idx in range(self.features_number):
            limits, classes = zip(*sorted(zip(X[:, idx], y)))
...
class Question:
    ...
    def __repr__(self):
        ...
        condition = "=="
        print("column: {} -- condition: {} --- header : {} --- value : {}".format(self.column, condition, header, self.value))
...
class DecisionTreeClassifier:
    ...
    def fit(self, training_data, labels_data):
        train = copy.deepcopy(training_data)
        labels = copy.deepcopy(labels_data)
        ...
        self.tree = build_tree(train)
        # print_tree(self.tree)

    def predict(self, test_data):
        for row in test_data:
            print("Actual: %s. Predicted: %s" %(row[-1], print_leaf(classify(row, self.tree))))
