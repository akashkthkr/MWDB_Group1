...
class SVMTask1():
    def __init__(self, train_data = None, test_data = None, task_id =None):
        ...
        self.type_ids_list = {"cc", "con", "emboss", "jitter", "neg", "noise01", "noise02", "original", "poster", "rot",
                         "smooth", "stipple"}
    ...
    def execute(self):
        ...
        count = 1
        for original_type_id in self.type_ids_list:
            print(count)
            count = count + 1
        ...
        jsonFile = open(OUTPUTS_PATH+"SVM_Classifier_Values_Task1.json", "w")
        jsonFile.write(jsonString)
        jsonFile.close()

        jsonString = json.dumps(negative_image_assosciation, cls=NumpyEncoder)
        jsonFile = open(OUTPUTS_PATH+"SVM_Classifier_negative_Values_Task1.json", "w")
        jsonFile.write(jsonString)
        jsonFile.close()

        jsonString = json.dumps(classifier_results, cls=NumpyEncoder)
        jsonFile = open(OUTPUTS_PATH+"SVM_Classifier_Results_Task1.json", "w")
        jsonFile.write(jsonString)
        jsonFile.close()
