...
class SVMTask3():
    ...
    def execute(self):
        ...
        count = 1
        for original_sample_id in self.image_sample_ids_list:
            print(count)
            count = count + 1
            ...
            for image_id in self.training_set_features:
                ...
                if image_sample_id == original_sample_id:
                    ...
                    if image_id in images_assosciation:
                        if any(original_sample_id in type for type in images_assosciation[image_id]):
                            for value in images_assosciation[image_id]:
                                value[original_sample_id] = max(value[original_sample_id], prediction_values[index])
                        else:
                            y = {original_sample_id: prediction_values[index]}
        ...
        jsonFile = open(OUTPUTS_PATH+"SVM_Classifier_Values_Task3.json", "w")
        jsonFile.write(jsonString)
        jsonFile.close()

        jsonString = json.dumps(negative_image_assosciation, cls=NumpyEncoder)
        jsonFile = open(OUTPUTS_PATH+"SVM_Classifier_negative_Values_Task3.json", "w")
        jsonFile.write(jsonString)
        jsonFile.close()

        jsonString = json.dumps(classifier_results, cls=NumpyEncoder)
        jsonFile = open(OUTPUTS_PATH+"SVM_Classifier_Results_Task3.json", "w")
        jsonFile.write(jsonString)
        jsonFile.close()

        svm_utilities = SVMUtilities(self.false_positives, self.true_positives, self.false_negatives,
                                     self.true_negatives, self.image_sample_ids_list, self.task_id, self.test_set_features, classifier_results)
