...
def get_all_features(image):
    ...
    cm = np.concatenate((np.histogram(mean)[0], np.histogram(sd)[0], np.histogram(skew)[0]))
    elbp = get_elbp(image)
    hog = get_hog(image)
    combined = np.concatenate((cm, elbp, hog))
    return {"combined": combined, "cm": cm, "elbp": elbp, "hog": hog}
...
def get_hog(image):
    ...
    value = hog(image, orientations=9, pixels_per_cell=(8, 8), cells_per_block=(
        2, 2), visualize=False, multichannel=False, block_norm="L2-Hys")
    # print("hog_value=",value,"sum=",np.sum(value),"min=",np.min(value),"max=",np.max(value))
    # value=value*np.sum(value)
    # value=value*np.max(value)
    value = np.asarray(value * 100, dtype=int)
    # print("new hog_value=", value, "sum=", np.sum(value), "min=", np.min(value), "max=", np.max(value))
    return value
